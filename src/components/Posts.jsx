    ...
    <div className={`${styles['posts-container']} container-fluid`}>
      <div className={`${styles['posts-content']} container bg-white px-4`}>
        ...
        <div className="d-flex flex-column mt-3">
          {(posts.slice(page * 5, page * 5 + 5)).map((post, index) => (
        ...
        <ul className="pagination d-flex justify-content-between mt-4 px-3">
          <li className="page-item">
            <button className="page-link" aria-label="Previous" onClick={() => setPage(prev => (prev - 1))} disabled={(page <= 0)}>
              <span aria-hidden="true">&laquo;</span>
              <span className="sr-only">Previous</span>
            </button>
          </li>
          <li className="page-item d-flex justify-content-center align-items-center">
            Page {page + 1} out of {posts.length / 5}
          </li>
          <li className="page-item">
            <button className="page-link" aria-label="Next" onClick={() => setPage(prev => (prev + 1))} disabled={page >= posts.length / 5 - 1}>
